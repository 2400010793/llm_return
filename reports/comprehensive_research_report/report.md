...
| Prompt | 配置 | 毛 bp/日 | 成本 bp/日 | 净 bp/日 | 平均持仓 | 新多头信号/日 | 换手/日 | 净年化 | 净 Sharpe |
|---|---|---:|---:|---:|---:|---:|---:|---:|---:|
| 盈利 | RoBERTa masked / 多空验证目标 | 3.65 | 0.56 | 3.10 | 101.8 | 6.14 | 5.58% | 6.63% | 0.468 |
| 收益 | BGE-M3 masked / 多空验证目标 | 4.01 | 0.55 | 3.46 | 89.9 | 6.14 | 5.53% | 7.68% | 0.537 |
| 超额收益 | RoBERTa masked / 多空验证目标 | 4.13 | 0.56 | 3.57 | 103.6 | 6.14 | 5.61% | 7.83% | 0.526 |
| 亏损 | BGE-M3 masked / RankIC 验证目标 | 4.18 | 0.55 | **3.63** | 97.4 | 6.14 | 5.51% | **8.30%** | **0.595** |

这 2,082 个交易日每天平均原始新闻信号为 14.86 个，进入新多头的只有 6.14 个；但
`gamma=0.1` 和“无信号继续持仓”使账面累积到约 90--104 只股票。因此软聚类结果同时反映
token 排序、收益簇映射和慢调仓三部分，不能把 3.10--3.63 净 bp 全部归因于聚类。
...
同时 `gamma=0.1` 将换手压到约 5.5%、成本压到约 0.55 bp/日。平均持仓约 94 只，而非
硬聚类模拟的 2.8 只。因此软聚类组合更平滑、更分散，但现有文件没有“同一慢调仓执行器下
