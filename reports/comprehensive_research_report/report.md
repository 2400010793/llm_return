...
3. **Token 的模型结构差异是真实的，但不能直接等同于理解能力。** 新公平 token 因子中，
   新浪 RoBERTa/BGE-M3/Qwen 的四 Prompt 平均相关为 0.678/0.877/0.663，巨潮为
   0.692/0.891/0.657。BGE-M3 最平滑，Qwen 在保留 Prompt 差异的同时取得最高 RankIC；
   RoBERTa 也有较低相关，但差异没有转化为同等预测力。
...
7. **三模型四 Prompt token 比较已完成，但树聚合没有稳定增量。** 巨潮 Qwen 最佳单
   Prompt/验证期选择树的平均 RankIC 为 0.05274/0.05275，新浪 2026 为
   0.05649/0.05459；Qwen 在两个数据源上都领先。BGE-M3 的四 Prompt 因子相关最高，
   但 RoBERTa/BGE-M3 的树模型均弱于最佳单 Prompt。该结果支持 Prompt token 因子存在
   结构差异，不支持“固定树模型优于简单组合”，也尚未完成四 Prompt token/body 配对。
...
巨潮整体覆盖 2010--2026 和同一批 816 只研究股票，但筛选口径在 2018 年发生变化：
2010--2017 抓取这 816 只股票的**全部公告**，2018--2026 使用这 816 只股票的
**重点公告**。2010--2017 采集审计覆盖 816 股票 x 8 年，共 6,528 个股票年且全部完成，
原始完成文件有 554,451 条记录。采集器保存公告索引、详情状态和 PDF；后续才进行 PDF
文本抽取、document ID 去重、清洗、分类与收益对齐。最终 903,665 行是两个口径合并后的
清洗面板，不是全时期全部公告，也不是原始下载量。
...
| 当前完整历史审计 | **816** | 候选池与清洗公告/收益标签可对齐的交集；两个时期股票范围相同 |
...
当前 816 只股票的 2010--2017 全部公告由 `run_cninfo_historical_local.sh` 分年执行，每个
股票单独输出 JSON、详情状态和 PDF；完成文件原始记录 554,451 条，清洗合并后为
553,088 行。2018--2026 使用同一批 816 只股票的重点公告，清洗面板为 350,577 行。
两段合计 903,665 行，不能理解为“本次爬虫一次下载了 903,665 份 PDF”，也不能称为
2010--2026 全时期全部公告。清洗阶段统一进行 PDF 文本抽取、公告 URL/document ID 去重、
正文长度过滤、分类和收益对齐；如要扩展到全 A 股，需要先
...
| 巨潮分段口径合并面板 | 2010-01-03 至 2026-07-31 | 903,665 | 816 | 2010--2017 全部公告 + 2018--2026 重点公告；pooled/Prompt、估值和风险标签 |
...
75,894 条面板的行号。三模型四 Prompt 已完成的冻结 embedding 交集为新浪 35,576 条、
巨潮 254,544 条；二级树模型使用一级 PCA32-Ridge 完全样本外预测形成的股票日，而不是
直接把这两个新闻行数当作回归样本量。

### 3.3 年度分布

| 年份 | 新浪全量 | 巨潮合并面板 | 旧新浪对齐 |
|---:|---:|---:|---:|
...
新浪 2026 年占比明显上升。巨潮 2010--2017 数量明显高于 2018 年以后，不只是自然年度
差异，还因为前段为全部公告、后段为重点公告。这个 2018 年筛选断点必须通过年度滚动、
逐年指标、公告类型占比和固定公告池诊断，不能把全期随机切分作为主结果，也不能把前后
数量变化解释为公司披露行为本身。
...
巨潮合并面板 903,665 条只覆盖同一批 816 只股票，且 2018 年前后公告筛选口径不同。
因此“公告条数大”不等于“横截面更广”，也不等于每一年都覆盖全部公告。
新浪覆盖 5,466 只股票，但不同年份和来源的有效覆盖不均。报告中的模型比较必须同时冻结
新闻行、股票日、标签可用性和测试年份。

### 3.5 从原始文本到可交易日期：清洗与对齐合同

#### 新浪：源文章与股票-文章行分开计数

新浪原始清洗输入有 714,555 行，其中 714,549 行可进入解析。经过无效时间、日期范围和
URL 日期冲突、乱码、指数/行情/专题页、短正文等规则后，接受 **167,713 篇源文章**。
股票识别只接受清洗后标题/正文中的明确六位代码或无歧义公司名，不把页面自带股票链接
单独当作真值。一篇新闻可以明确涉及多只股票，按股票展开后得到 **796,553 个研究行**、
5,466 只股票。因此 167,713 与 796,553 描述不同层级，不是两版互相冲突的样本数。

网页清洗删除现代页面头、显示发布时间外壳、导航、分享、编辑/推广尾注和异常替换字符；
正文至少 120 字。去重顺序是 `source_article_id+stock_id`、`url+stock_id`、
`stock_id+cleaned_text_hash`，既删除同一股票的重复文章，也保留同一新闻对不同股票的
独立研究行。最终重新生成唯一 `document_id`、稳定 `row_index` 和正文 hash。

#### 巨潮：903,816 中间档不等于 903,665 冻结面板

巨潮先记录详情/PDF 成功或失败状态，再抽取文本并清除页码、HTML、URL、PDF/浏览器样板、
替换字符和异常空白。通用合并清洗档从 913,014 行按 `stock_id+text_hash` 删除 9,198
行后得到 **903,816** 行；相同文本属于不同股票时仍保留。正式研究和中性 embedding 没有
直接采用该档，而是冻结两个已审计时期包：2010--2017 全部公告 553,088 行，加
2018--2026 重点公告 350,577 行，合计 **903,665** 行。

151 行差异目前只能确认是版本/选择合同不同，尚未完成逐行原因审计。严谨做法是把
903,665 固定为当前论文面板，保留两套 manifest 并单独做 row-level diff；在审计完成前，
不能把 903,816 替换进 embedding，也不能猜测差异来自“重复”或“短文本”。

#### 公告日、首个可交易日与标签

所有证券代码补齐为六位，首位为 5/6 的证券映射 `.SH`，其余映射 `.SZ`。新浪时间先按
UTC 解析，再转为 `Asia/Shanghai` 本地时间；巨潮优先使用 `published_at`，缺失时使用
`announcement_date`。大量历史公告只有日期级 `00:00+08:00`，因此主面板采用统一的
保守规则，而不声称知道盘中精确发布时间：

```text
event_center_date = 公告自然日当日或之后的首个交易所交易日
entry_date        = 严格晚于公告自然日的首个交易所交易日
next_day_return   = entry_date 对应的日收益
O2O               = entry_date 开盘至下一交易日开盘
```

周末和节假日按真实交易日历顺延。该主合同不使用“收盘前公告当天可交易”的 cutoff；它牺牲
一部分时间精度来避免日期级公告被当成盘前已知信息。`event_return_3d` 复合 center-1、
center、center+1，可能包含公告前交易日，只能解释为事件标签，不能作为公告后可部署的
三日收益。O2O 只有两个开盘价都存在且通过 IPO/可交易检查才保留。

估值使用 backward as-of，只允许最近的 `available_at <= published_at` 记录进入新闻行；
未来估值或当前最新估值不能回填历史。下一日波动率、价差和流动性先在 ticker 内显式
`shift(-1)`，再按股票/日期合并。模型侧的 scaler、PCA、聚类器和回归器仍只在当前历史
fold 拟合，从数据点时和模型训练两层控制前视。
...
RoBERTa 为 batch 8、最大 512 token，BGE-M3 为 batch 4、最大 1,000 token。下表来自
Slurm 已完成 array element 的 `ElapsedRaw`，排队时间未混入计算时间。

| 数据/模型 | 完成分片 | 单分片中位数 | P90 | 最大值 | 已观察 array 活跃墙钟 |
|---|---:|---:|---:|---:|---:|
| 新浪 RoBERTa | 256/256 | 46分50秒 | 3时04分36秒 | 3时32分12秒 | **4时28分17秒，完整** |
| 巨潮 RoBERTa | 255/256 | 1时11分10秒 | 2时01分47秒 | 4时33分41秒 | 9时16分57秒，基础 array 缺 shard-225 |
| 新浪 BGE-M3 | 189/256 | 5时15分51秒 | 9时07分35秒 | 15时19分15秒 | 15时49分45秒，仅部分任务 |
| 巨潮 BGE-M3 | 78/256 | 4时36分44秒 | 4时54分07秒 | 6时42分05秒 | 8时28分01秒，仅部分任务 |

RoBERTa 新浪整组是完整生产实测。巨潮基础 array 有一个失败分片，必须把 shard-225 补跑
产物纳入 manifest 后再称 256/256 完整。BGE-M3 两组分别取消 67 和 178 个未完成任务，
已保留 588,049 和 221,551 行产物；表中的墙钟只是已运行任务窗口，**不是全量完成时间**。

容量估算应先确定有效并发，再按 wave 计算：

```text
计算墙钟 ~= ceil(256 / 有效并发数) × 代表性单分片用时 × (1 + I/O/验收余量)
总周转时间 = Slurm 排队等待 + 计算墙钟
```

健康 CPU 节点和 256 个并发槽位都可用时，采用 P90 并加至少 20% I/O/验收余量，BGE-M3
单个全量 array 的条件预算约为新浪 **10--16 小时**、巨潮 **5--7 小时**；有效并发只有
64 时约需四个 wave。这个预算不是 SLA，因为节点速度差异和共享 Lustre I/O 已经使新浪
BGE 单 shard 从 1.45 小时分散到 15.3 小时。

Qwen 的完整参考 job 3651805 使用一张 RTX 4090，57,741 行 pooled embedding 用时
18,715 秒，即 5时11分55秒、约 3.085 行/秒。按同吞吐，**单次 pooled pass** 线性外推
为新浪 71.7 小时、巨潮 81.4 小时。该 job 只输出单个 4,096 维 pooled 向量，不是六
Prompt 的 token/full/body 完整合同；输入长度、GGUF runner、token 落盘、缓存和 GPU 数量
都会改变速度，故不得把单次外推乘六后写成正式工期。完整审计见
...
### 6.3 三模型四 Prompt Token 的公平比较

新实验只使用 `盈利/收益/超额收益/亏损` 的完整目标 span，每个模型分别在历史训练折拟合
PCA32，再以固定 Ridge 产生一级样本外因子；不做原始 768/1024/4096 维回归。二级模型
只使用同一模型自己的四个 token 因子，不混入其他模型，也不使用正文表示。`best_single`
由验证期 RankIC 选择，不是测试集事后最优；`tree_selected` 同样只按验证期从 XGBoost、
LightGBM、CatBoost 中选择。

二级模型必须使用历史一级 OOS 因子，不能用一级模型样本内预测补齐。因此新浪严格二级测试
只能使用 2026：2018--2023 拟合、2024--2025 验证、2026 测试；巨潮使用 2024--2026
三个测试年，每折为 2 年拟合、1 年验证、1 年测试。巨潮表中是三个测试年的等权平均。

| 数据源 | 模型 | 二级方法 | RankIC | RankIC IR | Top20 bp/日 | 多空 bp/日 |
|:--|:--|:--|--:|--:|--:|--:|
| 巨潮 | RoBERTa | 最佳单 Prompt | 0.033685 | 0.212895 | 87.19 | 85.71 |
| 巨潮 | RoBERTa | 四 Prompt 等权 | 0.029541 | 0.195932 | 84.47 | 82.89 |
| 巨潮 | RoBERTa | 验证期选择树 | 0.017032 | 0.107773 | 76.91 | 68.35 |
| 巨潮 | BGE-M3 | 最佳单 Prompt | 0.032889 | 0.215397 | 90.76 | 88.94 |
| 巨潮 | BGE-M3 | 四 Prompt 等权 | 0.029953 | 0.198075 | 90.34 | 89.25 |
| 巨潮 | BGE-M3 | 验证期选择树 | 0.026654 | 0.185772 | 85.66 | 82.47 |
| 巨潮 | Qwen | 最佳单 Prompt | 0.052742 | 0.323949 | 91.89 | 95.32 |
| 巨潮 | Qwen | 四 Prompt 等权 | 0.046754 | 0.297340 | 91.52 | 92.32 |
| 巨潮 | Qwen | 验证期选择树 | 0.052754 | 0.333080 | 83.59 | 90.69 |
| 新浪 | RoBERTa | 最佳单 Prompt | 0.008839 | 0.038513 | -4.74 | 32.14 |
| 新浪 | RoBERTa | 四 Prompt 等权 | -0.018896 | -0.073211 | -14.63 | 11.71 |
| 新浪 | RoBERTa | 验证期选择树 | -0.011770 | -0.049399 | -19.27 | 11.52 |
| 新浪 | BGE-M3 | 最佳单 Prompt | 0.018494 | 0.077935 | 1.51 | 15.60 |
| 新浪 | BGE-M3 | 四 Prompt 等权 | 0.032178 | 0.141993 | 6.99 | 30.14 |
| 新浪 | BGE-M3 | 验证期选择树 | 0.016845 | 0.063456 | -3.98 | 20.95 |
| 新浪 | Qwen | 最佳单 Prompt | 0.056492 | 0.251086 | 27.82 | 70.95 |
| 新浪 | Qwen | 四 Prompt 等权 | 0.052728 | 0.223121 | 25.66 | 90.91 |
| 新浪 | Qwen | 验证期选择树 | 0.054594 | 0.249149 | 24.72 | 74.45 |

Qwen 在两个数据源上都取得最高 RankIC，证明其四个目标 token 至少形成了较强的收益排序
因子；但树模型没有稳定创造增量。巨潮 Qwen 树相对最佳单 Prompt 的 RankIC 只增加
0.000012，多空反而下降 4.62 bp/日；巨潮 RoBERTa/BGE-M3 树的 RankIC 分别下降
0.01665/0.00624。新浪 Qwen 树也低于最佳单 Prompt，而 BGE-M3 的简单等权明显高于树。

四 Prompt 因子的逐日横截面 Spearman 进一步解释了组合差异：

| 模型 | 新浪 2026 | 巨潮 2024--2026 | 当前解释 |
|:--|--:|--:|:--|
| RoBERTa | 0.6779 | 0.6919 | Prompt 排序有差异，但预测较弱，不能把低相关直接解释为有用细粒度 |
| BGE-M3 | 0.8771 | 0.8908 | 四词排序最接近，更偏向共同收益语义 |
| Qwen | 0.6630 | 0.6573 | 保留较多 Prompt 差异，同时得到最高收益 RankIC |

这一结果支持“BGE-M3 更平滑、Qwen 更能保留细分差异”的描述，但还不满足“Qwen 已经
证明兼顾共同语义和可交易细粒度”的强结论：四 Prompt token 尚未和各模型正文做同折配对，
也没有跨模型融合和日期区块 bootstrap。新浪仅有 67 个有效 RankIC 日；巨潮有 625 日。
有效日期平均横截面约 53.4 只，Top20 平均约 11 只。表中 bp 未扣成本、涨跌停、容量和
卖空约束，只能用于同股票池的模型比较，不能作为可部署业绩。
...
巨潮 pooled embedding 使用 903,665 条合并公告记录、RoBERTa/BGE-M3、short 和
masked-short，共 72 个一级样本外回归配置，测试覆盖 2018--2026。这里的输入面板是
2010--2017 全部公告与 2018--2026 重点公告的合并，不是全时期全部公告。由于 6+2+1
滚动窗口跨过 2018 年筛选断点，早期训练集和后期测试集的公告类型分布不同；跨年为正
说明信号在该实际数据合同下可重复，不等于对“全公告口径”稳定。
...
巨潮合并面板有 90 万余条公告记录，却只覆盖同一批 816 只股票，且公告范围从全部公告
切换为重点公告；新浪股票更广但来源、长度和
...
- 不能把已完成的新浪 35,576 和巨潮 254,544 条 embedding 交集直接当作树模型的股票日样本量；
- 不能把已完成的三模型 token 比较写成 token/body 或跨模型融合已经完成；
...
| Qwen 四 Prompt token 同时保留差异并有较强收益排序 | 新浪/巨潮因子相关 0.663/0.657；RankIC 0.05649/0.05274，均领先 | 支持 token 层描述；未完成正文和跨模型融合，不能升级为完整理解结论 |
...
1. 在已冻结的新浪 35,576 条、巨潮 254,544 条交集上补齐同折 `body_mean/article_mean`，完成 token/body 配对。
2. 对现有 PCA32 token OOS 因子补做日期区块 bootstrap、逐年增量、Top20 重合和标准因子残差化。
3. 在基础 OOS 因子上做 Ridge、ElasticNet 和 HistGradientBoosting 跨模型 late fusion；固定树模型不再扩网格。
...
短语，会产生可重复、样本外、具有经济含义的不同横截面排序。历史表中 RoBERTa/BGE-M3
目标 token 的跨 Prompt Spearman 约为 **0.653/0.892**；新公平 token 因子在新浪为
**0.678/0.877**，在巨潮为 **0.692/0.891**，方向一致。Qwen 新结果为新浪 **0.663**、
巨潮 **0.657**，并在两个数据源上取得最高 RankIC。这说明模型确实产生不同的 Prompt
排序，但还不能单独说明“理解”或加入 Prompt 的因果效应。
...
当前新浪 BGE-M3 等权超过验证期最佳单 Prompt，但其他模型/数据源不一致；树模型也没有
稳定增量，因此证据停留在“相关但不同的 OOS 因子”，尚未达到独立可交易因子的验收线。
...
|---|---|---|---|
| RoBERTa | 双向 encoder，Prompt 在正文前，hidden size 768 | 新 token 因子相关：新浪 0.678、巨潮 0.692；树模型弱于最佳单 Prompt | 区分度较高但预测较弱，差异可能包含词面敏感 |
| BGE-M3 | 双向多语 encoder，Prompt 在正文前，hidden size 1024 | 新 token 因子相关：新浪 0.877、巨潮 0.891；三模型中最高 | 共享收益语义最强，但固定树没有利用出额外增量 |
| Qwen3-Embedding-8B | 因果 embedding，Prompt 在文章后，hidden size 4096 | 新 token 因子相关：新浪 0.663、巨潮 0.657；两个数据源 RankIC 最高 | `article_mean` 与后置 Prompt 信息流分离；需补四 Prompt 正文配对和跨模型融合 |
...
1. 在已完成的三模型四 Prompt token 共同样本上补生成 body/no-prompt 的同折 OOS 因子。
2. 在同一标签上完成跨模型残差、日期区块 bootstrap 和冻结 late fusion，进一步回答“是否形成独立因子”。
...
- 巨潮当前合并面板只覆盖 816 只股票，且 2010--2017 为全部公告、2018--2026 为重点
  公告；不代表无偏全 A 股公告历史，也不能把 `full` 文件名解释为全时期全公告。
- RoBERTa/BGE-M3 是双向编码器，Qwen 是因果模型；Prompt 位置影响机制不同。
- Qwen 已完成四 Prompt token 因子比较，但 token/body 对照仍只有单一“收益”Prompt，不能代表四 Prompt 平均正文增量。
...
5. 尚未运行完成的设计、token/body 和跨模型融合研究假设。
